<?php

$lang['eng'] = array(
    // pm-eng
    // profil-de
    "absender" => "Sender",

    // user-eng
    "alter" => "Age",

    // pm-eng
    // profil-de
    "antworten" => "Reply",

    "Antworten" => "Replies",
    
    // admin-eng
    // anwaerter-de
    "anwerter" => "	Awaiting Activation",

    // profil-eng
    // user-de
    // user-eng
    "avatar" => "Avatar",

    // de-eng-default
    // download-de
    "back" => "back",

    // pm-eng
    // profil-de
    "betreff" => "Subject",

    // anwaerter-eng
    "bewebungstext" => "Application",

    // anwaerter-eng
    "bewerbungen" => "Applications",

    // user-eng
    "birthday" => "Birthday",

    // de-eng-default
    // download-de
    "by" => "by",

    // pm-eng
    // profil-de
    "cantdelmessage" => "You can not delete this message!!",

    // profil-eng
    // user-de
    "changepw" => "Change Password",

    // profil-eng
    // user-de
    "clan" => "Clan",

    // profil-eng
    // user-de
    "clanhist" => "Clan History",

    // profil-eng
    // user-de
    "clanhp" => "Clan Homepage",

    // profil-eng
    // user-de
    "clanname" => "Clanname",

    // profil-eng
    // user-de
    "clantag" => "ClanTag",

    // de-eng-default
    // download-de
    "comments" => "Comments",

    // profil-eng
    // user-de
    "cpu" => "CPU",

    // user-eng
    "datum" => "Date",

    // de-eng-default
    // download-de
    "delet" => "Delet",

    // admin-eng
    // anwaerter-de
    // anwaerter-eng
    "delok" => "Successfully deleted!",

    // download-eng
    // pm-de
    "description" => "Description",

    // pm-eng
    // profil-de
    "doublepost" => "Double Post discovered!",

    // de-eng-default
    // download-de
    "edit" => "Edit",

    // admin-eng
    // anwaerter-de
    "editnews" => "Edit News",

    // de-eng-default
    // download-de
    "editprofile" => "Edit Profile",

    // profil-eng
    // user-de
    "email" => "E-Mail Adress",

    // pm-eng
    // profil-de
    "empfaenger" => "Receiver",

    // de-eng-default
    // download-de
    "error" => "Error",

    // admin-eng
    // anwaerter-de
    "error-del" => "Error while deleting:",

    // profil-eng
    // user-de
    "error-login-profil" => "You must login to change your profile!",

    // admin-eng
    // anwaerter-de
    "error-post" => "Error while posting",

    // pm-eng
    // profil-de
    "Fuer" => "For",

    // pm-eng
    // profil-de
    "gelesen" => "read",

    // profil-eng
    // user-de
    "graka" => "Graphics card",

    // profil-eng
    // user-de
    "hdd" => "Hard Drive",

    // admin-eng
    // anwaerter-de
    "headline" => "Headline:",

    // anwaerter-eng
    "kberechacanwae" => "You do not have the required permissions to activate members!",

    // anwaerter-eng
    "kberechdeanwae" => "You do not have the necessary authority to reject members!",

    // user-eng
    "lastlogin" => "Last Login",

    // de-eng-default
    // download-de
    "loggedas" => "Logged as:",

    // de-eng-default
    // download-de
    "login" => "Please Login!",

    // de-eng-default
    // download-de
    "loginbutt" => "Login",

    // de-eng-default
    // download-de
    "loginfail" => "Can`t Login, your Username or Password are wrong!",

    // user-eng
    "mail" => "E-Mail",

    // pm-eng
    // profil-de
    "messagenotforyou" => "This message is not intended for you!",

    // de-eng-default
    // download-de
    "morenews" => "More Articels",

    // user-eng
    "nachname" => "Family name",

    // user-eng
    "name" => "Name",

    // user-eng
    "never" => "Never",

    // admin-eng
    // anwaerter-de
    "newnews" => "Write News report:",

    // pm-eng
    // profil-de
    "newpm" => "Write new pm",

    // profil-eng
    // user-de
    "newpw" => "New Password",

    // de-eng-default
    // download-de
    "no" => "No",

    // de-eng-default
    // download-de
    "nodb" => "The Database do not exsist.",

    // admin-eng
    // anwaerter-de
    "nodelnews" => "You are not allowed to delete news!",

    // de-eng-default
    // download-de
    "noright" => "You are not allowed to do this!",

    // de-eng-default
    // download-de
    "noright2" => "You ae not allowed to enter this site!",

    // download-eng
    // pm-de
    "norightlookfile" => "You do not have the necessary level this download this file!",

    // profil-eng
    // user-de
    "notallfields" => "You have to fill out all mandatory fields",

    // de-eng-default
    // download-de
    "notfound" => "404 The requestet page can not be found!",

    // admin-eng
    // anwaerter-de
    "nowritenews" => "You are not allowed to write news!",

    // profil-eng
    // user-de
    "ok" => "Confirm",

    // profil-eng
    // user-de
    "ok_profil" => "Successfully Updated",

    // profil-eng
    // user-de
    "oldpw" => "Old Password",

    // user-eng
    "page" => "Page",

    // profil-eng
    // user-de
    "page-intern" => "Page Intern",

    // de-eng-default
    // download-de
    "password" => "Password",

    // profil-eng
    // user-de
    "persoenliches" => "Personal",

    // user-eng
    "pm" => "Send PM",

    // admin-eng
    // anwaerter-de
    "post" => "Post",

    // pm-eng
    // profil-de
    "postausgang" => "Outbox",

    // pm-eng
    // profil-de
    "posterror" => "Post Error Log",

    // admin-eng
    // anwaerter-de
    // anwaerter-eng
    "postok" => "Successfully registered",

    // profil-eng
    // user-de
    "pwwrong_profil" => "Password is entered incorrectly",

    // admin-eng
    // anwaerter-de
    "questions-webmaster" => "If you have any questions, contact the the webmaster!",

    // profil-eng
    // user-de
    "ram" => "RAM",

    // user-eng
    "rank" => "Level",

    // de-eng-default
    // download-de
    "register" => "Register",

    // profil-eng
    // user-de
    "repnewpw" => "Confirm Password",

    // de-eng-default
    // download-de
    "send" => "Send",

    // user-eng
    "since" => "Since",

    // profil-eng
    // user-de
    "sonstiges" => "Other",

    // profil-eng
    // user-de
    "start" => "Start",

    // profil-eng
    // user-de
    "telephone" => "Telephone or mobile phone",

    // pm-eng
    // profil-de
    "text" => "Text",

    // download-eng
    // pm-de
    "titel" => "Titel",

    // admin-eng
    // anwaerter-de
    "upload" => "Upload file",

    // profil-eng
    // user-de
    "upload-picture" => "Upload Picture",

    // download-eng
    // pm-de
    "uploadedam" => "Uploaded",

    // download-eng
    // pm-de
    "uploadedby" => "Uploaded by",

    // de-eng-default
    // download-de
    "username" => "Username",

    // de-eng-default
    // download-de
    "userorpasswordwrong" => "No connection possible! Username or Password wrong!",

    // pm-eng
    // profil-de
    "Von" => "By",

    // profil-eng
    // user-de
    // user-eng
    "wohnort" => "Residence",

    // admin-eng
    // anwaerter-de
    "wrotenews" => "If you have just written a news report, open in new window and login to your account.",

    // admin-eng
    // anwaerter-de
    "wrotenews2" => "Then go back to this page and press Update!",

    // de-eng-default
    // download-de
    "yes" => "Yes",
    
    'Login' => 'Login',
    'Logout' => 'Logout',
    'Passwort vergessen' => 'Forgot password',
    'Administration' => 'Administration',
    'PM-Menu' => 'PM-Menu',
    'Umfragen' => 'Votes',
    'Benutzte Bilder' => 'Used Pictures',
    'Unterseiten Verwalten' => 'Manage Subpages',
    'Rechte' => 'Rights',
    'Rechte-Verwaltung' => 'Right-Management',
    'Konfiguration' => 'Configuration',
    'Plugin System' => 'Plugin System',
    'Widget System' => 'Widget System',
    'Api-Schnittstelle' => 'Api-Interface',
    'Hier stehen Informationen zu der Api-Schnittstelle.' => 'All Informations about the API-Interface.',
    'Diese Seite zeigt nur Informationen und dient nicht zur Konfiguration.' => 'You can not modify settings on this page.',
    'Um die Einstellungen zu veraendern, gehen Sie in das Konfigurations bzw. in das Rechte System.' => 'To change this settings, you have to do this on the Config- or the Right-Management page.',
    'Status der Api Schnittstelle' => 'Status of the API-Interface',
    'Level, die ueber die Schnittstelle zugreifen koennen' => 'Level, that are allowed to access the Interface',
    'Benutzer, die ueber die Schnittstelle zugreifen koennen' => 'User, that are allowed to access the Interface',
    'Shared Secret' => 'Shared Secret',
    'Warnung' => 'Warning',
    'Diesen Text nicht an dritte weitergeben. Sollte es zu einer unberechtigten Verbindung kommen, dann die key.php im Ordner include/api loeschen!' => 'Keep this text secret. If you notice any unallowed access, delete key.php in include/api!',
    'Aktiviert' => 'Activated',
    'Deaktiviert' => 'Deactivated',
    'Termin hinzufuegen oder aendern' => 'Add or change Event',
    'Termin hinzufuegen' => 'Add Event',
    // l10n: %s -> Date | am 15.12.2012
    'am %s' => 'on %s',
    'Name' => 'Name',
    'Start-Zeit' => 'Start time',
    'End-Zeit' => 'End time',
    'Anzeigen auf' => 'Display on',
    'Benoetigtes Level' => 'Necessary Level',
    'End-Datum' => 'End date',
    'Bitte waehlen Sie einen Tag auf der linken Seite aus' => 'Please select a date on the left side',
    'Moechten Sie das Event wirklich loeschen?' => 'Do you realy want to delete this Event?',
    'ID' => 'ID',
    'Titel' => 'Titel',
    // l10n:    %s -> Start-Date
    //          %s -> Start-Time
    //          %s -> End-Date
    //          %s -> End-Time
    'Vom %s (%s) bis zum %s (%s)' => 'From %s (%s) until %s (%s)',
    'Sie haben nicht die benoetigte Berechtigung!' => 'You do not have the required permission!',
    'Das angegebene Datum ist nicht valid' => 'The specified date is not valid',
    'Fehlerhafte Daten' => 'Corrupted data',
    'Erfolgreich erstellt' => 'Successfully created',
    'Das End-Datum muss nach dem Start-Datum liegen' => 'The end date must be after the start date',
    'Keine Seite zum Anzeigen gewaehlt' => 'No pages selected for viewing',
    // l10n:    diese (Unterseite) ...
    'Sie koennen diese ueber den Unterseiten-Editor erstellen' => 'You can create these over the undersides Editor',
    'Dieser Kalender-Eintrag existiert nicht!' => 'This calendar entry does not exist!',
    'Erfolgreich aktualisiert' => 'Successfully updated',
    'Event erfolgreich geloescht' => 'Event deleted successfully', 
    'Kategorie' => 'Category',
    'Einstellung' => 'Setting',
    'Download-Bereich' => 'Download area',
    'Page not found' => 'Page not found',
    'Bitte Geben Sie zur ueberpruefung folgende Daten ein' => 'Please enter the following data for verification',
    'Bitte geben Sie ihr Passwort ein' => 'Please enter your password',
    'Passwort bestaetigung' => 'Password confirmation',
    // l10n:    %s -> date | geschrieben am 15.02.12
    'geschrieben am %s' => 'written on %s',
    'Ueberschrift' => 'Headline',
    'Level' => 'Level',
    'Die Berechtigungen koennen in der Seite "Rechte" geaendert werden' => 'The permissions can be changed in the page "rights"',
    'Level 0 bedeutet oeffentlich' => 'Level 0 means public',
    'Picture List oeffnen' => 'open Picture List',
    'Picture List' => 'Picture List',
    'Moechten Sie die Newsmeldung wirklich loeschen?' => 'Do you realy want to delet this News?',
    'Ersteller' => 'Author',
    'Version' => 'Version',
    // l10n:    Information: Test
    'Information' => 'Information',
    // l10n:    <img src="./images/lock.gif" alt="lock" width="12" height="12">
    'Plugins, die mit einem %s markiert sind, koennen nicht geaendert werden. Das (E) bedeutet, dass dieses Plugin extern, also z.B. ueber ein Template, eingebunden wurde.' => 'Plugins that are marked with a %s can not be changed. The (E) means that this plugin is externally, eg was implemented by using a template.',
    'Check all' => 'Check all',
    'Uncheck all' => 'Uncheck all',
    'Als gelesen makieren' => 'Mark as read',
    'Registrierung' => 'Registration',
    'Geschlecht' => 'Gender',
    'Maennlich' => 'Male',
    'Weiblich' => 'Female',
    'Pflichtfeld' => 'Eequired',
    'Maximal 20 Zeichen' => 'Maximum of 20 characters',
    'Mindestens 4 Zeichen' => 'At least 4 characters',
    'Template Name' => 'Template Name',
    'Zugewiesene Unterseite' => 'Assigned sub-page',
    'Eine neue Unterseite erstellen' => 'Create a new sub-page',
    'Liste der Vorhandenen Unterseiten' => 'List of existing sub-pages',
    'Template' => 'Template',
    'Unterseiten bearbeiten' => 'Edit sub-page',
    'Datei-Manager' => 'File Manager',
    'Dateien bearbeiten' => 'Edit file',
    'Datei hochladen' => 'Upload files',
    'Kategorie erstellen' => 'Create category',
    'Kategorien bearbeiten' => 'Edit category',
    // l10n:    Hochladen als: Admin
    'Hochladen als' => 'Upload as',
    'Veraendern von Dateien nicht moeglich!' => 'Files can not changed!',
    'Benutzer' => 'User',
    // l10n:    %s -> Username | Profil von Admin
    'Profil von %s' => 'Profile of %s',
    'Benutzerinformation' => 'User Information',
    'System' => 'System',
    'Neue Umfrage' => 'New Poll',
    'Umfrage bearbeiten' => 'Edit Poll',
    'Neue Umfrage starten' => 'Start new voting',
    'Bitte Geben Sie einen Titel ein' => 'Please enter a title',
    'Bitte Geben Sie mindestens 2 Antworten an' => 'Please enter at least 2 answers',
    'Ueberpruefen Sie die Eingaben' => 'Check the input',
    // l10n:    Aktuelle Auswahl: 15.02.2012
    'Aktuelle Auswahl' => 'current Selection',
    'Bitte beachten Sie die Meldungen' => 'Please note the messages',
    // l10n:    vote - noscript
    'Ueberpruefung deaktiviert' => 'verification disabled',
    // l10n:    %s -> Number of Votes
    'Bereits %s Stimmen abgegeben' => 'Already %s votes cast',
    'Ergebnisse' => 'Results',
    'Zum Abstimmen muessen Sie sich einloggen' => 'You have to login, to be able to vote',
    'Abstimmung abgelaufen' => 'Voting ended',
    'Bereits abgestimmt' => 'already voted',
    // l10n:    %s -> Count of Votes
    'Basierend auf %s Stimmen' => 'Based on %s votes',
    'Moechten Sie die Umfrage wirklich entfernen?' => 'Do you want to remove the Poll?',
    'Gueltig bis' => 'Valid to',
    'Moechten Sie die Datei wirklich loeschen?' => 'Do you want to delete this file?',
    'Die gewuenschte Datei existiert nicht!' => 'The requested file does not exist!',
    'Diese Seite kann nur direkt aufgerufen werden!' => 'This page can only be accessed directly!',
    'Der angegebene Code scheint schon benutzt worden sein oder ist abgelaufen!' => 'The specified code seems to invalid or expired!',
    'Die angegebenen Passwoerter stimmen nicht ueberein!' => 'The specified passwords do not match!',
    'Passwort erfolgreich geaendert!' => 'Password changed successfully!',
    'Benutzername nicht vorhanden' => 'Username does not exist',
    'Die Kontaktemail Adresse ist falsch' => 'The contact email address is incorrect',
    'Der angegebene Sicherheitscode stimmt nicht ueberein!' => 'The specified security code does not match!',
    'Ihre Registration wurde erfolgreich abgeschlossen' => 'Your registration was successfully completed',
    'Vielen Dank' => 'Thank you',
    'Ihre E-Mailadresse wurde bereits verifiziert!' => 'Your e-mail address has allready been verified!',
    'Gesperrt' => 'Closed',
    'Als gelesen markiert' => 'Mark as read',
    'Kein Betreff angegeben' => 'No subject given',
    'Der Benutzername muss mindestens 4 Zeichen haben' => 'The user name must have at least 4 characters',
    'Das Passwort muss mindestens 4 Zeichen haben' => 'Your password must be at least 4 characters long',
    'Benutzername oder Email bereits vorhanden' => 'Username or Email already exists',
    'Benutzername bereits vorhanden' => 'Username already exists',
    'Antrag erfolgreich gestellt' => 'Application is submitted successfully',
    'Eine Bestaetigungs E-Mail ist zu Ihnen Unterwegs' => 'A confirmation e-mail is on the way',
    'Bitte warten Sie, bis Sie freigeschaltet werden' => 'Please wait until you are unlocked',
    'Unterseite erfolgreich erstellt' => 'Sub-page successfully created',
    'Das gewaehlte Template ist nicht verfuegbar' => 'The chosen template is not available',
    'Es existiert bereits eine Seite mit diesen Namen' => 'There already exists a page with this name',
    'Die Zeichen &uuml; &ouml; &auml; und &szlig; sind im Namen nicht erlaubt' => 'The characters &uuml; &ouml; &auml; und &szlig; are not allowed in the name',
    'Die gewuenschte Unterseite ist nicht verfuegbar' => 'The requested sub-page is not available',
    'Unterseite erfolgreich modifiziert' => 'Sub-page successfully modified',
    'Liste der zugeordneten Unterseiten' => 'List of allocated sub-pages',
    'Liste der Vorhandenen Unterseiten' => 'List of existing sub-pages',
    // l10n:    %s -> Subpage name
    'Unterseite %s erfolgreich geloescht' => 'Sub-page %s successfully deleted',
    // l10n:    %s -> Subpage name
    'Moechten Sie die Unterseite %s wirklich loeschen?' => 'Do you realy want to delete the sub-page %s?',
    'Sie haben keine Berechtigung, Daten hoch zu laden' => 'You do not have permission to upload files',
    // l10n:    %s -> File Size in MB
    'Die gesendete Datei war zu groß! (Maximalgroeße: %sMB)' => 'The uploaded file was too big! (maximum size: %sMB)',
    'Dieser Datei-Typ ist nicht erlaubt' => 'This file type is not allowed',
    'Fehler beim Schreiben der Daten in die Datenbank' => 'Error writing data to the database',
    'Sie koennen keine Kategorien loeschen, in denen sich Dateien befinden' => 'You can not delete a category, in which there are files',
    'Moechten Sie die Kategorie wirklich loeschen?' => 'Do you really want to delete this category?',
    'Der Benutzer existiert nicht' => 'The user does not exist',
    'Bitte benutzen sie eines der folgenden Formate' => 'Please use one of the following formats',
    'Neue Newsmeldung verfassen' => 'Create a new News-Message',
    'Newsmeldungen Bearbeiten' => 'Edit News-Messages',
    'News' => 'News', 
    'Forum' => 'Board',
	'Moechten Sie dieses Element loeschen?' => 'Do you want to delete this entry?',
    
    '' => ''

);


?>