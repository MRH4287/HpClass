...
$loader = $hp->pluginloader;
if (!$loader->isEnabled("ForumPlugin"))
{
    $error->error("Für diese Seite ist das Plugin \"Forum\" notwendig!");
} else
{

  $forum = $loader->getPlugin("ForumPlugin");
  ...
  $sql = "SELECT * FROM `$dbprefix"."forums` WHERE `ID` = ".$get['forum'];
  ...
  $sql = "SELECT * FROM `$dbprefix"."posts`";
  $erg = $hp->mysqlquery($sql);
  ...
  // -.--------------------------------------------------------------------------------------------------------------------
  $threads = array();
  $sql = "SELECT ID, type FROM `$dbprefix"."threads` WHERE `forumid` = '$forumid' ORDER BY `lastpost` DESC";
  $erg = $hp->mysqlquery($sql);
  while ($row = mysql_fetch_object($erg))
  {
  ...
  $sql = "SELECT ID FROM `$dbprefix"."threads` WHERE `ID` = '$ID'";
  $erg = $hp->mysqlquery($sql);
  while ($row = mysql_fetch_object($erg))
  {
  $i++;
   ...
   ?>
    ...
    <tr>
      <td><table width="100%" border="0">
        ...
        <tr>
          <td>von <?php 
          $sql = "SELECT * FROM `$dbprefix"."user` WHERE `ID` = '".$row->userid."'";
      ...
      <td><table width="100%" border="0">
        <tr>
          ...
          {
          echo "von ";
          $sql = "SELECT * FROM `$dbprefix"."user` WHERE `ID` = '".$lastpost[$row->ID]."'";
          $erg2 = $hp->mysqlquery($sql);
          $row2 = mysql_fetch_object($erg2);
  ...
  $i = 0;
  $sql = "SELECT ID FROM `$dbprefix"."posts` WHERE `threadid` = '$threadid'";
  $erg = $hp->mysqlquery($sql);
  while ($row = mysql_fetch_object($erg))
  {
  $i++;
  ...
  {
  $sql = "SELECT * FROM `$dbprefix"."posts` WHERE `ID` = $ID";
  $erg = $hp->mysqlquery($sql);
  $row = mysql_fetch_object($erg);
  ...
  $sql = "UPDATE `$dbprefix"."threads` SET `lastpost` = '".time()."' WHERE `ID` = $threadid";
  $erg = $hp->mysqlquery($sql);
  ...
  $sql = "SELECT * FROM `$dbprefix"."forums` WHERE `ID` = $threadid";
  $erg = $hp->mysqlquery($sql);
  $row = mysql_fetch_object($erg);
  ...
  $sql = "SELECT * FROM `$dbprefix"."forums` WHERE `ID` = $forumid";
  $erg = $hp->mysqlquery($sql);
  $row = mysql_fetch_object($erg);
  ...
  $sql = "UPDATE `$dbprefix"."threads` SET `closed` = '$closed' WHERE `ID` = $id";
  $erg = $hp->mysqlquery($sql);
  ...
  if ($right[$level]['forum_edit_post'])
  {
  $sql = "DELETE FROM `$dbprefix"."threads` WHERE `ID` = '$threadid'";
  $erg = $hp->mysqlquery($sql);
  
  $sql = "DELETE FROM `$dbprefix"."posts` WHERE `threadid` = '$threadid'";
  $erg = $hp->mysqlquery($sql);
  ...
  if ($right[$level]['forum_edit_post'])
  {
  $sql = "DELETE FROM `$dbprefix"."posts` WHERE `ID` = '$postid'";
  $erg = $hp->mysqlquery($sql);
  ...
  if ($right[$level]['forum_del_forum'])
  {
  $sql = "DELETE FROM `$dbprefix"."forums` WHERE `ID` = '$postid'";
  $erg = $hp->mysqlquery($sql);
  
  $sql = "SELECT * FROM `$dbprefix"."threads` WHERE `forumid` = '$postid'";
  $erg = $hp->mysqlquery($sql);
  while ($row = mysql_fetch_object($erg))
  {
  $threadid = $row->ID;
  ...
  $sql = "DELETE FROM `$dbprefix"."threads` WHERE `forumid` = '$postid'";
  $erg = $hp->mysqlquery($sql);
  ...
  $sql = "SELECT * FROM `$dbprefix"."threads`";
  $erg = $hp->mysqlquery($sql);
  while ($row = mysql_fetch_object($erg))
  {
  ...
  foreach ($forums as $forumid=>$threads) {
  	...
  	foreach ($threads as $key=>$value) {
   	...
   	$sql = "SELECT * FROM `$dbprefix"."posts` WHERE `threadid` = '$value'";
   	$erg = $hp->mysqlquery($sql);
   	while ($row = mysql_fetch_object($erg))
   	{
    ...
    <tr>
      ...
      <td><table width="100%" border="0">
        <tr>
          ...
          {
          $sql = "SELECT * FROM `$dbprefix"."threads` WHERE `ID` ='".$lastthread[$row->ID]."'";
          $erg2 = $hp->mysqlquery($sql);
          $row2 = mysql_fetch_object($erg2);
